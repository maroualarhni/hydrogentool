# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 15:04:09 2026
...
def build_T1_ressources():
    ...
    data = {
        ...
        # Calibration techno-économique basée sur Hamza El Hafdaoui et al., 2023
        # Plage LCOE PV utility-scale Maroc : 30–50 $/MWh
        ...
        # Calcul CF via Weibull et LCOE CAPEX/CF
        # Sources : Manwell et al., 2009; CESE 2020; IRENA 2025
 ...
 # Paramètres turbine
        ...
        'consommation_eau_L_kgH2'   : [21.1]*6,  # constante physique : 9 L/kgH2 + pertes
        ...
        'connexion_reseau_elec'     : ['Bonne','Excellente','Excellente','Excellente','Excellente','Excellente'],
    ...
    # ── Conversion USD → EUR pour toutes les colonnes pertinentes ─────────
    cols_usd = [
...
def build_T3_stockage():
    ...
    # ===== Paramètres technologies stockage =====
    rows = [
        # H2 COMPRIMÉ 350 bar
    ...
    df_technologies['valeur_min_EUR']  = df_technologies.apply(lambda r: round(r['valeur_min']*TAUX_CHANGE['USD'],6) if mask_usd[r.name] else r['valeur_min'], axis=1)
    df_technologies['valeur_mode_EUR'] = df_technologies.apply(lambda r: round(r['valeur_mode']*TAUX_CHANGE['USD'],6) if mask_usd[r.name] else r['valeur_mode'], axis=1)
    df_technologies['valeur_max_EUR']  = df_technologies.apply(lambda r: round(r['valeur_max']*TAUX_CHANGE['USD'],6) if mask_usd[r.name] else r['valeur_max'], axis=1)
    df_technologies['devise_originale'] = df_technologies['unite'].apply(lambda u: 'USD' if 'USD' in str(u) else 'N/A')
    ...
    # Sauvegarde CSV technologies
    df_technologies.to_csv(os.path.join(OUTPUT_DIR,"csv/T3_technologies_stockage.csv"), index=False, encoding='utf-8-sig')
    ...
    reservoir_types.to_csv(os.path.join(OUTPUT_DIR,"csv/T3_types_reservoirs.csv"), index=False, encoding='utf-8-sig')
...
# Source : IEA 2024, Hydrogen Council 2023, IEA 2019 via Collis & Schomäcker 2022
# ══════════════════════════════════════════════════════════════════════════════
def mode_optimal(distance_km, type_corridor):
    ...
    Règles basées sur IEA (2024) et Hydrogen Council (2023) et IEA 2019 via Collis & Schomäcker 2022
    """
...
def build_T4_transport():
    ...
    for origine, destination, type_c in CORRIDORS:
        ...
        # ✅ Calcul coût ici (PAS dans une autre boucle)
        cout_min, cout_max = calcul_cout(mode, dist)

        # ✅ Ajouter directement à rows
        rows.append({
        ...
        # Affichage clair
        print(
            f"  ✓ {origine:15} → {destination:15} | {dist:6.0f} km "
            f"| {mode:25} | {vecteur} "
            f"| {cout_min:.3f}–{cout_max:.3f} $/kg "
            f"| {round(cout_min*TAUX_CHANGE['USD'],3):.3f}–{round(cout_max*TAUX_CHANGE['USD'],3):.3f} €/kg"
        )
    ...
    df.to_csv(f"{OUTPUT_DIR}/csv/T4_corridors_transport.csv",
...
def build_T0_references():
    ...
    rows = [
        ...
        ['REF02','DOE Hydrogen Program','2024','USD','USA',
         'CAPEX & storage data',
         'https://www.hydrogen.energy.gov/'],

        ['REF03','Hydrogen Insights (Hydrogen Council Report)','2024','USD','Global',
         'Market projections',
         'https://hydrogencouncil.com/wp-content/uploads/2024/09/Hydrogen-Insights-2024.pdf'],

        ['REF04','IRESEN Morocco Study','2022','MAD','Morocco',
         'Geological storage',
         None],

        ['REF05','OSRM OpenStreetMap','2026','-','Global',
         'Distance calculation',
         'https://www.openstreetmap.org/'],

        # ===== SCIENTIFIC LITERATURE =====
        ['REF06','Reksten et al. (2022) - Electrolyser CAPEX model','2022','USD','Europe',
         'Electrolyser cost modelling',
         'https://doi.org/10.1016/j.ijhydene.2022.08.306'],

        ['REF07','Lu et al. (2025) - Hydrogen storage & transport cost review','2025','USD','Global',
         'Storage & transport cost drivers',
         'https://doi.org/10.1016/j.ijhydene.2025.01.196'],

        ['REF08','Yang et al. (2023) - Review of H2 storage technologies','2023','USD','Global',
         'Technology review',
         'https://doi.org/10.1093/ce/zkad021'],

        ['REF09','Dinh et al. (2024) - LCOT H2 & NH3','2024','USD','Global',
         'Transmission comparison',
         'https://doi.org/10.1016/j.ijhydene.2024.03.066'],

        ['REF10','Rezaei et al. (2024) - Atlas H2 MENA','2024','USD','MENA',
         'Site suitability & economics',
         'https://doi.org/10.1016/j.solener.2024.000203'],

        ['REF11','Department for Energy Security and Net Zero (2022) - Fugitive Hydrogen Emissions in a Future Hydrogen Economy',
         '2022','GBP','UK',
         'Hydrogen leakage & climate impact',
         'https://www.gov.uk/government/publications/fugitive-hydrogen-emissions-in-a-future-hydrogen-economy'],

        # ===== MOROCCO & PROJECT REPORTS =====
        ['REF12','MASEN Annual Report 2022 + World Bank PCR','2022','MAD','Morocco',
         'Project & financial data',
         'https://www.masen.ma/fr/presentation'],

        ['REF13','IRENA Renewable Power Generation Costs 2022','2022','USD','Global',
         'Renewable CAPEX benchmarks',
         'https://www.irena.org/publications/2022/Oct/Renewable-Power-Costs-in-2021'],

        ['REF14','MASEN 2019 + World Bank Group 2020','2020','MAD','Morocco',
        ...
        ['REF15','ONEE Annual Report 2022','2022','MAD','Morocco',
         'Electricity & grid data',
         'https://www.one.org.ma/FR/pages/interne.asp?esp=2&id1=10&id2=73&t2=1'],

        ['REF16','OCP Group Sustainability Report 2023','2023','USD','Morocco',
         'Industrial hydrogen strategy',
         'https://www.ocpgroup.ma/en/sustainability/sustainability-report-performance-data'],

        ['REF17','IRESEN Annual Report 2022','2022','MAD','Morocco',
         'R&D hydrogen projects',
         'https://iresen.org/'],

        ['REF18','RWE – Germany–Morocco Partnership 2023','2023','EUR','Germany/Morocco',
         'Green hydrogen partnership',
         'https://energypartnership.ma/'],

        ['REF19','MASEN Strategic Plan 2030','2023','MAD','Morocco',
         'Long-term energy strategy',
         'https://www.masen.org.ma/en/presentation'],

        ['REF20','H2Uppp Germany–Morocco 2023','2023','EUR','Germany/Morocco',
         'International cooperation',
         'https://www.giz.de/en/worldwide/107551.html'],

        ['REF21','OCP Sustainability Report 2023','2023','USD','Morocco',
    ...
    df.to_csv(
        f"{OUTPUT_DIR}/reports/T0_references.csv",
        index=False,
        encoding='utf-8-sig'
    )

    print(f"✓ T0 sauvegardé : {len(df)} références avec DOI/URL")
...
class MonteCarloH2Morocco:
    ...
    def run_LCOH(self, location='Ouarzazate', technologie='PEM', annee=2024):
        """Calcule la distribution LCOH pour une configuration donnee"""

        # Seed isole par configuration pour reproductibilite totale
        np.random.seed(hash((location, technologie, annee)) % (2**32))

        # Facteurs d'apprentissage selon l'annee (Wright's Law)
    ...
    'Guelmim'    : 0.1665,
        ...
        CF_h = CF_HYBRIDE_T1.get(location, 0.3)
...
def validate_model(mc_engine, projets_df):
    ...
    checks = [
        ...
        #5.5, 40),
        ('LCOH Ouarzazate PEM 2024',
         mc_engine.run_LCOH('Ouarzazate','PEM',2024)['P50'],
            5.5, 40),
        ('LCOH Dakhla 2030',
...
def fig1_lcoh_distributions(mc):
    ...
    regions = ['Ouarzazate', 'Laayoune', 'Dakhla', 'Tanger', 'Jorf Lasfar', 'Guelmim']
